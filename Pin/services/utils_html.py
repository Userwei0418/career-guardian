...
def intit_blacklist_tail():
    ...
    #读取black列表
    with open("data/black_re.txt",encoding="utf-8") as f:
        for _line in f.read().splitlines():
            _line = _line.strip()
            if _line:
                _blacklist.append(_line.strip())
    #读取black html列表
    with open("data/black_re_html.txt",encoding="utf-8") as f:
        for _line in f.read().splitlines():
            _line = _line.strip()
            if _line:
                _blacklist_html.append(_line.strip())
    #读取black html列表
    with open("data/black_md.txt",encoding="utf-8") as f:
        for _line in f.read().splitlines():
            _line = _line.strip()
            if _line:
                _blacklist_md.append(_line.strip())
    #读取black md footer列表
    with open("data/black_md_footer.txt",encoding="utf-8") as f:
        for _line in f.read().splitlines():
            _line = _line.strip()
            if _line:
