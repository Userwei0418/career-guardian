...
# on_resp_proc removed - module deleted
...
class SpiderCom():
    ...
    def __init__(self, _file="99", use_db=True):
        self.browser = None
        self.file = _file
        self.use_db = use_db
        ...
        self.progress_list = []
        if use_db:
            self.savepath_base = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
        else:
    ...
    def get_savepath(self,_tmp = "/data/tmp"):
        if self.savepath_base:
            return self.savepath_base + _tmp
        if os.name == "nt":
    ...
    #获取节点列表
    def get_nodes(self, company_ids=None):
        if self.use_db:
            return self._get_nodes_from_db(company_ids)
        nodes = {}
    ...
    def _get_nodes_from_db(self, company_ids=None):
        """从数据库加载公司节点（新 schema: json_config）"""
        from crawl_db import get_companies_by_ids, get_active_companies

        if company_ids:
            companies = get_companies_by_ids(company_ids)
        else:
            rows = get_active_companies()
            companies = {row["com_id"]: row for row in rows}

        nodes = {}
        for com_id, row in companies.items():
            cfg = row.get("json_config", {})
            node = {
                "com_name": row.get("com_name", ""),
                "com_webname": row.get("com_webname") or "",
                "com_logo": row.get("com_logo") or "",
                "urls": cfg.get("urls", {}),
                "pre_open_url": cfg.get("pre_open_url", ""),
                "json_domain": cfg.get("json_domain", ""),
                "func_name": cfg.get("func_name", ""),
                "template": cfg.get("template", ""),
                "table_selector": cfg.get("table_selector", ""),
                "table_selectors": cfg.get("table_selectors", ""),
                "detail_selector": cfg.get("detail_selector", ""),
                "detail_selectors": cfg.get("detail_selectors", ""),
                "click_text": cfg.get("click_text"),
                "click_type": cfg.get("click_type"),
                "max_parent_level": cfg.get("max_parent_level"),
                "page_count": cfg.get("page_count", 1),
                "click_load_more": cfg.get("click_load_more", ""),
                "detail_hd": cfg.get("detail_hd", ""),
                "data_proc_type": cfg.get("data_proc_type", ""),
                "hd_all_location": cfg.get("hd_all_location", ""),
            }
            nodes[com_id] = [node]
        return nodes
    ...
    #特殊on_response的执行
    # on_resp_proc removed - auto_on_response deleted
    ...
    def get_page_detail_data(self,page,_key,url,_k,key_tmp_dir,sch_info,_item):
        ...
        try:
            ...
            # 写入数据库记录
            try:
                from crawl_db import save_crawled_job
                save_crawled_job(
                    com_id=_key,
                    job_title=_item.get("announcement_name", "")[:255],
                    job_type=_k.split("_")[0],
                    source_url=_fullurl,
                    raw_html=_context_outtext,
                    raw_json=_item
                )
            except Exception as db_e:
                ner_logger.error(f"写入DB记录失败: {db_e}")
