...
def extract_table_from_html(htmlcontext, tempfile):
    ...
    for job in soup.find_all('li', class_='social_core_list_nub'):
        ...
        # 安全提取每一项
        hd_dept = safe_text(details[0].find('span') if len(details) > 0 else "")
        hd_loc = safe_text(details[2].find('span') if len(details) > 2 else "")
        hd_job_num = safe_text(details[1].find('span') if len(details) > 1 else "")
        publish_time = safe_text(details[3].find('span') if len(details) > 3 else "")

        # 岗位类别：存在 p 标签，格式为 “岗位类别：xxx”
        if len(details) > 0:
            p_tag = details[0].find('p')
            if p_tag and '：' in p_tag.text:
                hd_job_category = p_tag.text.split('：', 1)[1].strip()
            else:
                hd_job_category = ""
        else:
            hd_job_category = ""
