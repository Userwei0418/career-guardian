...
def extract_table_from_html(htmlcontext, tempfile):
    ...
    for item in job_items:
        ...
        hd_job_category = item.find('div', class_='dept-name').get_text(strip=True) if item.find('div', class_='dept-name') else ""  # Assuming no job category is provided in the HTML
        name_loc= f"{announcement_name}_{hd_loc}"
        print(name_loc)
        if name_loc in job_name_id:
            id = job_name_id[name_loc]
            link = f"https://careers.hellobike.com/#/jobDetail?id={id}&from=实习生招聘"
        else:
            link = ""
