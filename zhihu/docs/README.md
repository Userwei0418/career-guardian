...
- [成长守护详细设计、交互基线与实现状态](./growth-guardian.md)：按“当下的事—过去的果—未来的路”组织闭环；Phase A 的工作候选、1—3 项确认、结果/事件和周报已进入代码与 `0058`，但实际库仍为 `0056` 且没有成长表。明确 `/today` 只读取摘要，30/60/90 只是可选模板，代码通过不等于已部署或完整模块验收。
