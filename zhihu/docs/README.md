...
- [权益守护·劳动合同审查详细设计](./rights-guardian.md)：劳动者视角的合同上传、解析、条款审查、原文定位和记录管理。
- [收支守护详细设计、交互基线与实现状态](./income-guardian.md)：收入与支出双入口、候选核对、统一经济事实、工资到账、长截图 OCR、可信账本、图表、AI、KNOWLEDGE、导出、数据边界和真实实现状态。
- [成长守护详细设计、交互基线与实现状态](./growth-guardian.md)：按“当下的事—过去的果—未来的路”组织闭环；实际库与代码 head 均为 `20260826_0068`。用户授权清场完成时 30 张成长表为 0 行，随后重新操作已产生新数据；`/today` 只读取摘要，AI 只形成建议，代码通过不等于远端部署或完整业务验收。
...
- 历史参考项目已在能力、测试夹具和来源说明内化后从最新树移除；不再把整套第三方/历史源码随职护仓库分发。
