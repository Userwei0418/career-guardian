...
2026-08-17 当前快照：`zhihu` 有 132,804 条标准岗位、132,804 条岗位来源和 554 家标准企业；`market_raw` 从 Pin 的 816 条公司配置中归并出 630 家公司、拆分出 1,467 个招聘渠道，另保留职护原生的中国人保公司及其 3 个官方渠道。管理后台因此显示 631 家采集公司、共 1,470 个公司渠道和 7 类采集模板。42 个旧渠道因非 HTTPS、入口或生成解析器不完整被标为配置异常，不会自动运行。实际 Raw、晋级和隔离数量以管理后台实时统计为准；历史岗位数据仍完整保留在 `pin_legacy_staging`。
