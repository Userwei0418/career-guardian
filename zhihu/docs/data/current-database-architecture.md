...
├── data_sources（站点、适配器、分页、字段映射、审批与启停）
...
管理员启动来源采集前，该来源必须同时具备条款审批、启用状态、HTTPS 白名单和 `promotion_mapping` 产品字段映射。采集成功后，本次新 Raw 会自动进入映射与质量门：通过项标记 `promoted` 并写入 `zhihu.market_*`，不通过项标记 `quarantined` 并保存原因码。只有 `promoted` 才算进入产品库；异常中断时保留 `pending_gate` 供排查和重试，绝不把它当作合格岗位。
...
管理后台按“来源 → 任务 → Raw 去重 → 字段映射 → 质量门 → 主库”展示完整流程。每个历史任务固化读取数、Raw 新增数、重复数、晋级数、隔离数和失败数，不依赖前端临时计算。页面可编辑的只是不含凭据的技术配置；Cookie、Token、密钥和密码会被服务端拒绝保存。

2026-08-15 清理后的历史快照：`zhihu` 有 132,804 条标准岗位、132,804 条岗位来源、554 家企业、64 个标准城市、12 个岗位方向和 88,464 个技能标签；当时 `market_raw` 有 5 个来源配置、0 个采集任务、0 条新 Raw。2026-08-17 的采集改造将来源注册表调整为 4 个脱敏测试来源和 3 个中国人保官方来源；实际 Raw 与晋级数量以管理后台实时统计为准。历史数据仍完整保留在 `pin_legacy_staging`。
