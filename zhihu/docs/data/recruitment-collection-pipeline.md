...
任务状态、六阶段进度、事件日志、人工终止、渠道健康和 AI 调用观测的详细契约见[采集监控、日志与运维](./collection-observability-and-operations.md)。
...
11. **入口、列表、详情分层判定**：入口 404/410、列表解析失败、详情导航失败和详情正文失败是四种不同事实，不得统一伪装成“选择器失效”或“任务成功”。
12. **完整渲染 HTML 是 Raw 证据**：独立详情页必须保留完整渲染 HTML，同时保留可读正文和抓取元数据；正文才进入标准化/AI，HTML 用于追溯和重处理。
...
浏览器不会直接访问市场采集服务。管理页面请求先到职护后端，再由后端使用 `MARKET_INTERNAL_TOKEN` 调用市场服务内部接口。普通列表/任务接口不返回 Raw 原文；只有管理员显式请求单条 Raw 证据时，才由受控代理返回完整 HTML/正文并以转义文本展示。系统凭据任何情况都不返回浏览器。
...
单次覆盖不会回写渠道默认值。所有本次设置写入 `crawl_tasks.run_options`，请求值、生效值及其来源同时进入任务事件。渠道级最小请求间隔仍是安全下限，单次任务只能保持或放慢访问节奏，不能将其降低。
...
当前任务状态为 `pending`、`running`、`cancelling`、`cancelled`、`succeeded`、`failed`。管理员终止排队任务时会直接进入 `cancelled`；终止运行任务时先进入 `cancelling`，执行器在页面等待、翻页、详情等待和逐条 Raw 写入边界检查信号后收敛。终止前已经提交的 Raw 保留，主动终止不记为任务失败。
...
执行过程同时持久化六阶段进度：`entry_validation`、`job_discovery`、`detail_capture`、`raw_write`、`standardization_gate`、`completed`。岗位发现阶段总量尚未冻结，使用动态进度；列表边界确定后，详情、Raw 和准入阶段才显示确定分母。任务列表只显示一条总进度条，任务详情显示阶段指标和事件时间线。
...
9. 对每个岗位确认详情导航方式：独立链接、同域 URL 模板、可见标题点击或列表内展开；
10. 打开独立详情视图后保留完整渲染 HTML，再按配置选择器、`main/article/[role=main]/body`、iframe 顺序提取可读正文；
11. 记录实际导航策略、抓取模式、命中选择器、HTML/正文大小和警告；
12. 输出岗位、完整 Raw 证据、实际策略和传输元数据。
...
对异步 SPA，点击下一页后不能只等待固定几百毫秒就判断“无新增”。引擎会在有界时间内轮询外部岗位 ID 集合，确认页面数据确实变化后再进入下一轮；这避免按钮点击成功但仍读取旧 DOM，导致误停在首屏 10 条。
...
列表只负责发现索引，不再假设“列表卡片已是完整 JD”。适配器优先使用卡片的详情链接；没有 href 但有稳定 ID 时，可以用渠道配置的同域详情 URL 模板构造链接；两者都没有时，才使用可见岗位标题点击。例如 HotJob 可从卡片 `postId` 构造 `posDetail.html`；58 新站点使用 `#/job/` hash 路由；这些均受渠道主机白名单约束。

打开详情后，适配器保留完整渲染 HTML，然后提取正文。即使已经有列表标题和详情 URL，但零条岗位拥有有效详情证据时，整个批次仍必须失败，不得把“只抓了 URL”标为成功。

任务传输元数据同时记录详情完整数、详情抓取数、缺失数、抓取模式分布、命中选择器和警告分布。页面成功打开但只取得标题或 URL，不算完整采集成功；即使 Raw 已留痕，空职责、空要求且没有完整 JD 的记录也会被质量门隔离。

### 9.1 学校招聘公告统一进入岗位主库

学校就业网是岗位渠道之一。当前学校目录作为 `source_kind=school_announcement`、`raw_only=false` 的候选来源，仍保留学校公告的来源身份，但使用与企业渠道相同的岗位质量边界：

- 仅导入公开 URL、声明式规则和经安全静态检查的兼容解析器；
- 不复制旧项目凭据、Cookie、代理、运行时网络逻辑或服务依赖；
- 默认是 `disabled`、条款 `pending`，配置学校公告到统一岗位事实的 `promotion_mapping`；采集后先进入 Raw，标准化和质量门通过后才写入 `zhihu.market_*`；
- 含命令式点击、前置打开、自定义函数等旧动作的来源标记为待人工复核，不会被自动开启。
- 招聘单位必须来自公告中的明确证据；无法确认企业名称、详情正文不足或其他关键事实不可靠时保持隔离，不把学校名称猜成企业名称。

当前包装资产共 308 个学校配置、667 个公告来源和 85 个兼容解析器。2026-08-18 经管理员明确批准后已导入正式 MySQL；导入不会批量启用，`needs_review` 与 `invalid` 来源仍不可运行。新任务使用统一岗位管道，历史 Raw 不因目录升级自动重跑；来源当前启用数量以管理后台为准。
...
- 原始岗位 URL；
- 来源发布时间与抓取时间；
- 原始结构化数据、完整渲染 HTML、可读详情正文和标准化结果；
- 浏览器或 HTTP 传输元数据；
- 内容指纹和模式版本；
- 校验状态、处理状态、处理次数和错误；
- 第一次和最后一次观察时间。
...
MySQL 中 `raw_records.raw_text` 使用 `LONGTEXT`，以便保留大型 SPA 详情页的完整渲染 HTML。结构化 `raw_payload` 不再重复存一份 HTML，只保留详情正文、抓取模式、导航策略、选择器和警告等可查询元数据。

`raw_records.schema_version` 使用 `VARCHAR(80)`，容纳公司岗位和学校公告等明确的版本名称；正式迁移 `20260818_0015` 增加非空 JSON `crawl_tasks.run_options`，`20260818_0017` 增加非空 JSON `crawl_tasks.progress_snapshot`。正式运行和验收只使用 MySQL。
...
- 当结构化详情缺失但 `raw_text` 已保存完整 HTML 时，从 HTML 提取可读正文作为处理证据，并记录 `rendered_html_fallback`；
- 清理多余空白和空字段；
...
因此“保存了完整 HTML”说明采集证据已经存在，但不代表记录自动合格：程序提取不充分时应立即进入有来源证据约束的 LLM 语义兜底；如果 LLM 仍无法生成可回指原文的完整字段，记录继续被质量门隔离。若本次失败同时形成新的声明式长期策略，该候选只有在回放和 Canary 的解析结果通过质量门后才允许管理员审批替换，不能自动发布。
...
| --- | --- | ---: | ---: | --- |
| `entry_invalid` | degraded | 5 分钟 | 60 分钟 | 核对并更新 HTTPS 入口，不要只改选择器 |
| `list_parse` | degraded | 5 分钟 | 60 分钟 | 修复列表发现策略 |
| `detail_navigation` | degraded | 5 分钟 | 60 分钟 | 修复详情链接、同域模板、标题点击或 hash 路由 |
| `detail_content` | degraded | 5 分钟 | 60 分钟 | 详情页已打开，修复正文选择器或 iframe 提取 |
...
适配器底层保留更精确的错误码：`source_entry_invalid`、`list_parse_failed`、`detail_navigation_failed`和 `detail_content_failed`。任务失败事件同时记录底层错误类型、归一化健康类别和恢复建议，管理员无需从一条“解析失败”中猜根因。
...
本节只列出采集链路必须暴露的业务事实。完整状态机、阶段指标、事件码、取消语义和运维验收见[采集监控、日志与运维](./collection-observability-and-operations.md)。
...
- Raw HTML 字符/字节数、正文长度、抓取模式、详情策略、命中选择器和警告；
- 每条任务事件的可展开 JSON 上下文。

任务列表和详情默认只返回经过权限控制的预览和大小。管理员显式点击“查看 HTML/正文证据”时才按 Raw ID 请求完整证据；页面把 HTML 放在转义 `<pre>` 中，不注入 DOM、不执行脚本、不建立 iframe。该接口不得把凭据、Cookie、请求头或浏览器存储暴露给前端。
...
- `GET /internal/admin/raw-records/{raw_record_id}/evidence`

公司级和来源级启动接口支持本次 `browser_mode=default|headless|visible`、`collection_mode=default|full|incremental`、最大页数、最大记录数及详情随机等待范围。取消接口接收操作者和原因；Raw 证据接口只允许管理员通过业务后端代理访问，不是面向普通用户或公网的原文 API。
...
预览不写库。`--apply` 需要显式的 `MARKET_RAW_DATABASE_URL`，默认导入仍是 `disabled + pending + raw_only`。只有管理员已明确批准时，才允许附加 `--approve-and-enable --actor <管理员审计标识>`；该模式记录审批，但只启用 `configuration_status=ready` 的来源，`needs_review/invalid` 始终保持不可运行。
...
当前正式开发、运行和验收只使用 MySQL。2026-08-18 已按管理员要求删除北京时间 2026-08-17 起的采集数据、任务/处理/AI 日志和派生状态，并把所有来源恢复为未启用；旧验收报告和 SQLite 隔离脚本已移除，不能继续引用旧任务编号或数量作为当前证据。

重新验收必须从管理后台逐项启用明确来源并发起任务，随后核对入口有效性、翻页动作、详情完整 HTML/正文、随机等待、处理轨迹、质量门、Core 血缘和任务事件。清场工具 `market-data/scripts/reset_recent_collection_state.py` 默认只做影响预览，正式修改必须显式传入 `--apply`。
...
### 22.3 页面显示零职位，却报解析规则未命中

先检查官网是否有明确、可见的空状态元素和文本，例如“暂无职位”或“抱歉，暂时没有您要找的职位”。命中受限空状态选择器后应将任务记为成功零结果并记录 `source_empty_confirmed`；只有页面既没有岗位也没有可信空状态证据时，才按列表解析故障处理。

### 22.4 列表已出现新岗位，但市场总量没变
...
如果入口本身返回 404/410，应按 `entry_invalid` 处理：由管理员从企业/学校公开官网核对新 HTTPS 入口和 allowlist，然后再运行列表/详情验收。AI 选择器候选不负责猜测或自动发布新入口 URL。
...
- [采集监控、日志与运维](./collection-observability-and-operations.md)
