...
import { updateContractMetadata, reviewContract, fetchPerspectives } from "../api/contracts";
import {
  conditionTypeLabel,
  contractStatusLabel,
  providerLabel,
...
const activePerspective = ref("enterprise");
const perspectives = ref([
  { code: "enterprise", name: "企业对企业", description: "从企业角度审查" },
  { code: "individual", name: "个人对企业", description: "从个人角度审查" },
  { code: "worker", name: "劳动者", description: "从劳动者角度审查" },
]);

const activePerspectiveResult = computed(() => {
  const pr = contract.value?.perspective_results;
  if (pr && pr[activePerspective.value]) {
    return pr[activePerspective.value];
  }
  return contract.value?.review_result || null;
});

const activePerspectiveRisks = computed(() => activePerspectiveResult.value?.risks || []);
const activeRiskCount = computed(() => activePerspectiveRisks.value.length);
const activeRiskScore = computed(() => activePerspectiveResult.value?.risk_score ?? null);
const activeRiskGrade = computed(() => activePerspectiveResult.value?.risk_grade ?? null);
const activeRiskGradeColor = computed(() => riskGradeColorMap[activeRiskGrade.value] || "#909399");

function getPerspectiveScore(code) {
  const pr = contract.value?.perspective_results;
  if (pr && pr[code]) return pr[code].risk_score;
  if (code === "enterprise" && contract.value?.review_result) return contract.value.review_result.risk_score;
  return null;
}

function getPerspectiveGrade(code) {
  const pr = contract.value?.perspective_results;
  if (pr && pr[code]) return pr[code].risk_grade;
  if (code === "enterprise" && contract.value?.review_result) return contract.value.review_result.risk_grade;
  return null;
}

function isPerspectiveReviewed(code) {
  const pr = contract.value?.perspective_results;
  if (pr && pr[code]) return true;
  if (code === "enterprise" && contract.value?.review_result) return true;
  return false;
}

const riskCount = computed(() => activeRiskCount.value);
...
const highlightedRawText = computed(() => {
  const rawText = contract.value?.raw_text || "";
  if (!rawText || !activePerspectiveRisks.value.length) {
    return escapeHtml(rawText);
  }
  const riskColors = { high: "#FEE2E2", medium: "#FEF3C7", low: "#FEF9C3" };
  const riskBorderColors = { high: "#EF4444", medium: "#F59E0B", low: "#EAB308" };
  let html = escapeHtml(rawText);
  const sortedRisks = [...activePerspectiveRisks.value]
    .filter(r => r.matched_text && r.matched_text.length > 1)
    .sort((a, b) => (b.matched_text?.length || 0) - (a.matched_text?.length || 0));
  for (const risk of sortedRisks) {
    const escapedMatch = escapeHtml(risk.matched_text);
    const bgColor = riskColors[risk.level] || "#FEF9C3";
    const borderColor = riskBorderColors[risk.level] || "#EAB308";
    const tooltip = `${risk.title}: ${risk.description}`.replace(/"/g, "&quot;");
    const replacement = `<mark class="risk-highlight" style="background:${bgColor};border-bottom:2px solid ${borderColor}" title="${tooltip}">${escapedMatch}</mark>`;
    html = html.replace(escapedMatch, replacement);
  }
  return html;
});
...
async function runReview() {
  ...
  try {
    await reviewContract(route.params.id, activePerspective.value);
    const pName = perspectiveLabelMap[activePerspective.value] || activePerspective.value;
    ElMessage.success(`「${pName}」视角审查完成`);
    await fetchDetail();
  } catch (error) {
  ...
  <div v-loading="loading" class="page-stack">
    ...
    <el-card v-if="contract" class="detail-card">
      <template #header>
        <div class="card-header">
          <span>审查视角</span>
          <el-tag type="info">选择不同视角查看合同风险</el-tag>
        </div>
      </template>
      <div class="perspective-tabs">
        <div
          ...
          :key="p.code"
          class="perspective-tab-item"
          :class="{ active: activePerspective === p.code }"
          @click="activePerspective = p.code"
        >
          <div class="perspective-tab-header">
            <span class="perspective-tab-name">{{ p.name }}</span>
            <el-tag
              v-if="isPerspectiveReviewed(p.code)"
              size="small"
              :style="{ backgroundColor: riskGradeColorMap[getPerspectiveGrade(p.code)] || '#909399', color: '#fff', border: 'none' }"
            >
              {{ getPerspectiveScore(p.code) }}分 {{ getPerspectiveGrade(p.code) }}
            </el-tag>
            <el-tag v-else size="small" type="info">未审查</el-tag>
          </div>
          <p class="perspective-tab-desc">{{ p.description }}</p>
        </div>
      </div>
    </el-card>

    <el-card v-if="contract && activePerspectiveResult" class="score-gauge-card">
      <div class="score-gauge-layout">
        <div class="score-gauge-left">
          <div class="score-circle" :style="{ '--score-color': activeRiskGradeColor }">
            <svg viewBox="0 0 120 120" class="score-ring">
              <circle cx="60" cy="60" r="52" fill="none" stroke="var(--border-color)" stroke-width="8" />
              <circle cx="60" cy="60" r="52" fill="none" :stroke="activeRiskGradeColor" stroke-width="8"
                :stroke-dasharray="`${(activeRiskScore || 0) / 100 * 327} 327`"
                stroke-linecap="round" transform="rotate(-90 60 60)" class="score-progress" />
            </svg>
            <div class="score-number">{{ activeRiskScore ?? '--' }}</div>
            <div class="score-grade" :style="{ color: activeRiskGradeColor }">{{ activeRiskGrade || '-' }}</div>
          </div>
        </div>
        <div class="score-gauge-right">
          <h3>合同体检报告</h3>
          <p class="score-verdict" v-if="activeRiskScore !== null">
            <template v-if="activeRiskScore >= 85">✅ 这份合同整体健康，风险较低。继续保持关注即可。</template>
            <template v-else-if="activeRiskScore >= 70">🟡 合同基本可以，但有几处需要留意。建议重点看下方标注的风险项。</template>
            <template v-else-if="activeRiskScore >= 50">🟠 合同存在中等风险，建议逐条核对下方问题，必要时请专业人士协助。</template>
            <template v-else>🔴 合同风险较高！强烈建议仔细审查下方每一项风险，考虑寻求法律帮助后再签署。</template>
          </p>
          <p class="score-summary" v-if="activePerspectiveResult.summary">
            <strong>守护助手说：</strong>{{ activePerspectiveResult.summary }}
          </p>
          <div class="score-stats">
            <span>共 <strong>{{ activeRiskCount }}</strong> 项风险</span>
            <span class="dot">·</span>
            <span>高风险 <strong style="color: var(--color-danger)">{{ activePerspectiveRisks.filter(r => r.level === 'high').length }}</strong></span>
            <span class="dot">·</span>
            <span>中风险 <strong style="color: var(--color-warning)">{{ activePerspectiveRisks.filter(r => r.level === 'medium').length }}</strong></span>
            <span class="dot">·</span>
            <span>低风险 <strong style="color: var(--color-success)">{{ activePerspectiveRisks.filter(r => r.level === 'low').length }}</strong></span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <el-card v-if="contract" class="detail-card">
      <template #header>
        <div class="card-header">
          <span>模型参与状态</span>
          <el-tag type="info">{{ perspectiveLabel(activePerspective) }}视角</el-tag>
        </div>
      </template>
        ...
        <el-card class="detail-card">
          <template #header>
            <div class="card-header">
              <span>合同基础信息</span>
              <div>
                <el-tag v-if="activePerspectiveResult" type="info" style="margin-right: 8px">{{ perspectiveLabel(activePerspective) }}</el-tag>
                <el-tag v-if="activePerspectiveResult" type="success">已提取</el-tag>
              </div>
            </div>
          </template>

          <el-descriptions :column="1" border v-if="activePerspectiveResult">
            <el-descriptions-item label="合同名称">{{ activePerspectiveResult.extracted_fields.contract_name || "-" }}</el-descriptions-item>
            <el-descriptions-item label="合同编号">{{ activePerspectiveResult.extracted_fields.contract_number || "-" }}</el-descriptions-item>
            <el-descriptions-item label="项目名称">{{ activePerspectiveResult.extracted_fields.project_name || "-" }}</el-descriptions-item>
            <el-descriptions-item label="甲方">{{ activePerspectiveResult.extracted_fields.party_a || "-" }}</el-descriptions-item>
            <el-descriptions-item label="乙方">{{ activePerspectiveResult.extracted_fields.party_b || "-" }}</el-descriptions-item>
            <el-descriptions-item label="合同类型">{{ activePerspectiveResult.extracted_fields.contract_type || "-" }}</el-descriptions-item>
            <el-descriptions-item label="签订日期">{{ activePerspectiveResult.extracted_fields.sign_date || "-" }}</el-descriptions-item>
            <el-descriptions-item label="合同金额">{{ activePerspectiveResult.extracted_fields.contract_amount || "-" }}</el-descriptions-item>
            <el-descriptions-item label="工期">{{ activePerspectiveResult.extracted_fields.construction_period || "-" }}</el-descriptions-item>
            <el-descriptions-item label="付款条款">{{ activePerspectiveResult.extracted_fields.payment_terms || "-" }}</el-descriptions-item>
            <el-descriptions-item label="质保期">{{ activePerspectiveResult.extracted_fields.warranty_period || "-" }}</el-descriptions-item>
            <el-descriptions-item label="争议解决">{{ activePerspectiveResult.extracted_fields.dispute_resolution || "-" }}</el-descriptions-item>
            <el-descriptions-item label="违约责任">{{ activePerspectiveResult.extracted_fields.breach_liability || "-" }}</el-descriptions-item>
          </el-descriptions>
        ...
        <el-card class="detail-card">
          <template #header>
            <div class="card-header">
              <span>风险展示区</span>
              <div>
                <el-tag :style="activeRiskScore !== null ? { backgroundColor: activeRiskGradeColor, color: '#fff', border: 'none', marginRight: '8px' } : {}" type="warning">
                  {{ activeRiskScore !== null ? `${activeRiskScore}分 ${activeRiskGrade}` : `${activeRiskCount} 项风险` }}
                </el-tag>
                <el-tag type="info">{{ perspectiveLabel(activePerspective) }}</el-tag>
              </div>
            </div>
          </template>

          <div v-if="activePerspectiveRisks.length" class="risk-list">
            <el-card
              v-for="risk in activePerspectiveRisks"
              :key="risk.code"
              class="risk-item"
              shadow="never"
            >
              ...
              <div v-if="risk.suggested_revision" class="suggestion-box">
                <div class="suggestion-header">
                  <span>💡 修改建议</span>
                  <el-button text size="small" @click="copySuggestion(risk.suggested_revision)">📋 复制</el-button>
                </div>
                <p class="suggestion-text">{{ risk.suggested_revision }}</p>
              </div>
              <div v-if="risk.plain_explanation" class="plain-explanation">
                <span class="plain-label">🗣️ 说人话：</span>{{ risk.plain_explanation }}
              </div>
    ...
    <el-card v-if="contract" class="detail-card">
      <template #header>
        <div class="card-header">
          ...
          <div>
            <el-tag v-if="activePerspectiveResult" type="info" style="margin-right: 8px">{{ perspectiveLabel(activePerspective) }}</el-tag>
            <el-tag v-if="activePerspectiveResult">{{ providerLabel(activePerspectiveResult.provider) }}</el-tag>
          </div>
        </div>
      </template>
      <p>{{ activePerspectiveResult?.summary || "尚未生成审查摘要" }}</p>
    </el-card>
    ...
    <el-card v-if="contract" class="detail-card">
      <template #header>
        <div class="card-header">
          <span>原始解析文本</span>
          <el-tag v-if="activePerspectiveRisks.length" type="warning">
            {{ activePerspectiveRisks.filter(r => r.matched_text).length }} 处高亮
          </el-tag>
        </div>
      </template>
      ...
      <div v-else class="raw-text highlighted-raw" v-html="highlightedRawText"></div>
    </el-card>
