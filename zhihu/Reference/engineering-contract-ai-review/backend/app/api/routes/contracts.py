...
from app.services.version_control_service import check_version_and_increment, release_lock
...
from app.services.document_parser_service import get_document_parser_service
...
from app.core.perspectives import DEFAULT_PERSPECTIVE, list_perspectives
...
async def upload_contract(
    ...
    if not file_bytes:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Upload failed: file is empty.")
    if len(file_bytes) > 50 * 1024 * 1024:  # 50MB limit
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Upload failed: file exceeds 50MB limit.")

    # Parse document using strategy-pattern parser service
    filename = file.filename or "unknown"
    parser_service = get_document_parser_service()
    parsed_content = parser_service.parse_document(file_bytes, filename)

    if parsed_content.parse_status == "failed":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=parsed_content.parse_error or "Failed to parse document.")
...
def review_contract(
    contract_file_id: int,
    perspective: str = Query(default=DEFAULT_PERSPECTIVE),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_contract_write_access),
) -> ContractReviewResponse:
    return review_contract_parse_result(
        db, contract_file_id, current_user, trigger_source="manual", perspective_code=perspective,
    )
