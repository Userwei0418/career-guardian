...
from app.core.perspectives import DEFAULT_PERSPECTIVE
...
from app.services.openai_compatible_llm_service import (
    ...
    build_openai_compatible_suggestions_with_settings,
    build_openai_compatible_plain_explanations_with_settings,
)
...
def generate_suggestions(
    ...
    """Generate specific modification suggestions for risks. Returns {risk_code: suggestion_text}."""
    runtime = resolve_llm_settings(db, settings)
    if runtime.review_provider != "openai_compatible":
        return {}
    try:
        return build_openai_compatible_suggestions_with_settings(
            OpenAICompatibleRuntimeSettings(
                llm_base_url=runtime.llm_base_url,
                llm_api_key=runtime.llm_api_key,
                llm_model=runtime.llm_model,
...
def generate_plain_explanations(
    ...
    """Generate plain-language explanations for risks. Returns {risk_code: explanation_text}."""
    runtime = resolve_llm_settings(db, settings)
    if runtime.review_provider != "openai_compatible":
        return {}
    try:
        return build_openai_compatible_plain_explanations_with_settings(
            OpenAICompatibleRuntimeSettings(
                llm_base_url=runtime.llm_base_url,
                llm_api_key=runtime.llm_api_key,
                llm_model=runtime.llm_model,
