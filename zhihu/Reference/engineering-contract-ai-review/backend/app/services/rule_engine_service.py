...
def _perspective_matches(rule: ReviewRule, perspective_code: str) -> bool:
    scopes = _parse_scope(getattr(rule, "perspective_scope", None))
    if not scopes:
        return True
    return perspective_code in scopes
...
def evaluate_review_rules(
    ...
    perspective_code: str = "enterprise",
) -> list[ReviewRiskItem]:
    ...
    for rule in rules:
        if not _scope_matches(rule, fields):
            continue
        if not _perspective_matches(rule, perspective_code):
            continue
