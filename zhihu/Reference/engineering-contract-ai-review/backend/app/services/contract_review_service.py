...
class BuiltinHeuristicRule:
    ...
    description: str
    recommendation: str
    perspective_codes: tuple[str, ...] = ("enterprise", "individual", "worker")
...
BUILTIN_HEURISTIC_RULES: tuple[BuiltinHeuristicRule, ...] = (
    ...
    BuiltinHeuristicRule(
        ...
        perspective_codes=("enterprise", "individual"),
    ),
    BuiltinHeuristicRule(
...
def identify_contract_risks(
    ...
    for rule in BUILTIN_HEURISTIC_RULES:
        if perspective_code not in rule.perspective_codes:
            continue
...
def compute_risk_score(risks: list[ReviewRiskItem]) -> tuple[int, str]:
    score = 100
    for risk in risks:
        if risk.level == "high":
            score -= 15
...
def generate_review_summary(
    ...
    return generate_review_summary_with_provider(db, fields, risks, perspective_code=perspective_code)
...
def review_contract_parse_result(
    ...
    logger.info("Reviewing contract %s, text length: %d chars, perspective: %s", contract_file_id, len(normalized_text), perspective_code)
    ...
    try:
        ...
        rule_risks = evaluate_review_rules(db, normalized_text, fields, perspective_code=perspective_code)
        logger.debug("  Found %d rule risks", len(rule_risks))
        logger.debug("Step 3: Identifying AI risks")
        ai_risks = identify_contract_risks(normalized_text, fields, perspective_code=perspective_code)
        ...
        llm_risk_result = generate_llm_risk_supplements(db, normalized_text, fields, base_risks, perspective_code=perspective_code)
        ...
        summary_result = generate_review_summary(db, fields, risks, perspective_code=perspective_code)
        ...
        logger.debug("Step 7: Computing risk score")
        risk_score, risk_grade = compute_risk_score(risks)
        logger.debug("  Risk score=%d, grade=%s", risk_score, risk_grade)
        logger.debug("Step 8: Generating modification suggestions")
        suggestions = generate_suggestions(db, normalized_text, risks, perspective_code=perspective_code)
        for risk in risks:
            if risk.code in suggestions:
                risk.suggested_revision = suggestions[risk.code]
        logger.debug("  Generated %d suggestions", len(suggestions))
        logger.debug("Step 9: Generating plain-language explanations")
        explanations = generate_plain_explanations(db, normalized_text, risks, perspective_code=perspective_code)
        for risk in risks:
            if risk.code in explanations:
                risk.plain_explanation = explanations[risk.code]
        logger.debug("  Generated %d explanations", len(explanations))
    ...
    existing_result = db.scalar(
        select(ContractReviewResult).where(
            ContractReviewResult.contract_file_id == contract_file_id,
            ContractReviewResult.perspective_code == perspective_code,
        )
    )
    ...
    if existing_result is None:
        ...
        existing_result.risk_grade = risk_grade
