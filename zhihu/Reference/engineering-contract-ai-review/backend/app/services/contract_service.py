...
def get_contract_detail(db: Session, contract_file_id: int, actor: User) -> ContractDetailResponse:
    ...
    review_result = db.scalar(
        select(ContractReviewResult).where(
            ...
            ContractReviewResult.perspective_code == "enterprise",
        )
    )

    # Query all perspective results
    all_review_results = db.scalars(
        select(ContractReviewResult).where(ContractReviewResult.contract_file_id == contract_file_id)
    ).all()
    perspective_results = {}
    for rr in all_review_results:
        perspective_results[rr.perspective_code] = {
            "review_result_id": rr.id,
            "perspective_code": rr.perspective_code,
            "latest_version_id": rr.latest_version_id,
            "latest_version_no": rr.latest_version_no,
            "provider": rr.provider,
            "extracted_fields": rr.extracted_fields,
            "risks": rr.risks,
            "summary": rr.summary,
            "risk_score": rr.risk_score,
            "risk_grade": rr.risk_grade,
    ...
    return ContractDetailResponse(
        ...
        review_result={
            ...
            "risk_grade": review_result.risk_grade,
            "created_at": review_result.created_at.isoformat(),
        }
        if review_result
        else None,
        perspective_results=perspective_results if perspective_results else None,
    )
