...
- 岗位查询与薪资/技能洞察已经拆开：辅助洞察失败时岗位列表仍然展示；页面顶部新增紧凑岗位列表并可定位到完整岗位详情。
...
- 市场数据：29 个单元/契约测试通过，包括质量门草稿/预检/发布、无效权重拒绝、Pin 迁移、Raw 与历史共用质量门、薪资单位清洗、Core 查询和上游不可用降级。
- 职护后端：29 个安全、事件、迁移、知识文章、市场网关、财务契约和连续案例测试通过。
