...
- 机会守护默认直接展示 Core 中前 20 条标准岗位，目标职能和城市改为可选筛选；个人档案仅用于技能匹配，不再阻塞岗位主查询。
...
- 列表每页只渲染 20 条紧凑岗位，不再在同一页面重复渲染一套完整详情；翻页只重新读取岗位，不重复请求薪资和技能洞察。
- 新增独立岗位详情页 `/opportunity/jobs/[jobId]`，展示岗位职责、任职要求、结构化技能、企业信息、个人技能差距、质量评分、准入策略版本、首次/最后观察时间和原始来源。
- 详情接口只读取通过质量门且有来源回链的 Core 岗位；不存在、未认证或无来源岗位不会进入详情页。
- “纳入我的机会守护”移动到详情页主行动，继续写入职业事件、岗位证据、机会结论和需人工确认的下一行动。

### 已完成：统一 MySQL 与 Pin 存量清洗迁移

- 开发和生产运行统一使用同一个 MySQL 实例；`zhihu` 保存全部职护业务数据与 27 篇知识文章，市场数据按 Staging、Raw、Core 三个逻辑库隔离。SQLite 仅用于自动化测试。
...
- 机会守护正式运行只读 `market_core`。上海“数据”查询返回 617 条历史岗位；12 个有效月薪样本的中位数为 27,500 元，并保留方法版本和来源。
- 统一迁移与启动脚本会拒绝运行时 SQLite；原本地预览 SQLite 和空的市场 Raw SQLite 已清除。

### 已完成：可配置岗位质量门

- `market_core` 新增版本化质量门策略表，当前 `career-guardian-job-core-v1` 保持生效并关联 132,804 条已认证岗位。
...
- 市场数据：31 个单元/契约测试通过，包括质量门草稿/预检/发布、Pin 迁移、Raw 与历史共用质量门、分页契约、岗位详情、薪资单位清洗、Core 查询和上游不可用降级。
...
- 浏览器机会/管理路径：机会守护默认展示 20 条岗位，接口总量为 132,804；管理员“数据准入”页可看到生效版本、132,804 条认证数以及草稿、预检、发布控件，页面无控制台错误。
