...
export default function GrowthPage() {
  const paths = [
    { href: "/growth/work", index: "01", eyebrow: "工作现场", title: "当下的事", description: "快速整理工作，标出建议优先事项；完成后形成待确认事件。", action: "进入当下的事" },
    { href: "/growth/assets", index: "02", eyebrow: "职业资产", title: "过去的果", description: "把已确认经历沉淀为作品、证据、反思与能力事实，保留来源和版本。", action: "查看过去的果" },
    { href: "/growth/direction", index: "03", eyebrow: "职业仪表盘", title: "未来的路", description: "从市场信号与本人目标形成方向假设，人确认后再转为行动。", action: "看清未来的路" },
  ];
  return <div className="space-y-8 pb-12">
    <section className="overflow-hidden rounded-3xl border border-[var(--color-border-light)] bg-white p-6 md:p-10">
      ...
      <div className="mt-4 grid gap-7 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
        <div><h1 className="text-3xl font-semibold leading-tight md:text-5xl">让每一天的工作，留下可验证的成长轨迹</h1><p className="mt-5 max-w-3xl leading-7 text-[var(--color-text-secondary)]">不是给成长打一个总分，而是把“当下的事、过去的果、未来的路”分开管理。系统可以整理候选，事实与方向必须由你确认。</p></div>
    ...
    <section aria-label="成长守护三个工作区" className="grid gap-5 lg:grid-cols-3">
      {paths.map((item) => <Link key={item.href} href={item.href} className="group flex min-h-72 flex-col rounded-3xl border border-[var(--color-border-light)] bg-white p-6 transition hover:-translate-y-1 hover:border-[var(--color-primary)] hover:shadow-lg focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--color-primary)]">
        <div className="flex items-center justify-between"><span className="text-xs font-semibold tracking-[0.18em] text-[var(--color-primary-dark)]">{item.index} · {item.eyebrow}</span><span aria-hidden="true" className="text-2xl text-[var(--color-text-muted)] transition group-hover:translate-x-1 group-hover:text-[var(--color-primary)]">→</span></div>
