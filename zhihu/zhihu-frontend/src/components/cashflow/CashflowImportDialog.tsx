...
function RecognitionProgressPanel({ batch, busy, onRetry }: { batch: CashflowImportBatch; busy: boolean; onRetry: (sequenceNumber: number) => void }) {
  ...
  const duplicateImages = Array.isArray(progress.duplicate_images) ? progress.duplicate_images : [];
  const slices = Array.isArray(progress.slices) ? progress.slices : [];
  const coverageWarnings = slices.filter((slice) => slice.row_coverage_status === "partial" || slice.row_coverage_status === "over_detected");
  ...
  return <section className="mt-5 rounded-2xl border border-sky-100 bg-sky-50/55 p-5" aria-label={isSequence ? "连续截图分片识别进度" : "长截图分片识别进度"}>
    ...
    {coverageWarnings.length > 0 && <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-900"><p className="font-semibold">有 {coverageWarnings.length} 个片段的交易行数需要核对</p><p>程序只负责检查可能漏识别或多识别，不会据此补造交易；请重试对应片段或结合候选逐笔确认。</p></div>}
    ...
    <div className="mt-4 flex flex-wrap gap-2">{slices.map((slice) => {
      const coverageNeedsReview = slice.row_coverage_status === "partial" || slice.row_coverage_status === "over_detected";
      ...
      return <div key={slice.sequence_number} className={`rounded-xl border px-3 py-2 text-xs ${className}`}><span>{sliceName} · {label}</span>{slice.status === "failed" && <button type="button" onClick={() => onRetry(slice.sequence_number)} disabled={busy} className="ml-2 font-semibold underline underline-offset-2 disabled:opacity-50">重试</button>}{slice.status === "completed" && slice.ocr_text_fully_processed && <p className="mt-1 leading-5 opacity-80">OCR 全文 {slice.ocr_processed_character_count ?? slice.ocr_character_count ?? 0} 字 · {slice.ocr_chunk_count ?? 1} 段均已处理</p>}{slice.status === "completed" && typeof slice.program_candidate_count === "number" && <p className="mt-1 leading-5 opacity-80">程序确定 {slice.program_candidate_count} 条 · AI 辅助 {slice.ai_candidate_count ?? 0} 条{slice.ai_chunk_count ? `（${slice.ai_chunk_count} 段）` : ""}</p>}{typeof slice.expected_transaction_rows === "number" && <p className="mt-1 leading-5 opacity-90">程序检测约 {slice.expected_transaction_rows} 行 · 已提取 {slice.recognized_candidate_count ?? 0} 条{slice.row_coverage_status === "partial" ? ` · 可能漏 ${slice.missing_transaction_rows ?? slice.expected_transaction_rows} 条` : slice.row_coverage_status === "over_detected" ? " · 可能存在多识别" : ""}</p>}{slice.error_message && <p className="mt-1 max-w-72 leading-5">{slice.error_message}</p>}</div>;
    })}</div>
  </section>;
}
