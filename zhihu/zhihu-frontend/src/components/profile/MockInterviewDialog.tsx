...
export default function MockInterviewDialog({ target, onClose }: { target: TargetForInterview; onClose: () => void }) {
  ...
  const answer = async () => {
    ...
    try {
      const started = await api.post<StartResponse>(`/opportunity/targets/${target.id}/mock-interviews`, { interview_type: type, difficulty, planned_duration_minutes: minutes });
