...
function PracticeTrend({ items }: { items: MockInterviewSession[] }) {
  ...
  const points = scores.map((score, index) => `${comparable.length === 1 ? 160 : 24 + (index * 272) / (comparable.length - 1)},${112 - score}`).join(" ");
  ...
  return <section className="card overflow-hidden">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold tracking-[0.16em] text-[var(--color-primary-dark)]">PRACTICE TREND</p><h2 className="mt-1 text-xl font-semibold">自我介绍能力曲线</h2><p className="mt-2 text-sm text-[var(--color-text-secondary)]">{latest.job_snapshot.title || "目标岗位"} · {latest.target_duration_seconds || 60} 秒 · 最近 {comparable.length} 次同口径练习</p></div><div className="rounded-2xl bg-emerald-50 px-4 py-3 text-right"><p className="text-xs text-emerald-800">最新综合表现</p><p className="mt-1 text-2xl font-semibold text-emerald-950">{scores[scores.length - 1]}<span className="text-sm"> / 100</span></p>{delta !== null && <p className={`text-xs ${delta >= 0 ? "text-emerald-700" : "text-amber-700"}`}>较上次 {delta >= 0 ? "+" : ""}{delta}</p>}</div></div>
    <div className="mt-5 overflow-x-auto rounded-2xl bg-[var(--color-bg-warm)] p-4"><svg viewBox="0 0 320 132" className="h-44 min-w-[520px] w-full" role="img" aria-label="自我介绍最近练习综合得分趋势"><line x1="24" y1="112" x2="296" y2="112" stroke="#d9ded9"/><line x1="24" y1="62" x2="296" y2="62" stroke="#e8ebe7" strokeDasharray="4 4"/><polyline points={points} fill="none" stroke="#3f9185" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/>{scores.map((score, index) => { const x = comparable.length === 1 ? 160 : 24 + (index * 272) / (comparable.length - 1); const y = 112 - score; return <g key={comparable[index].id}><circle cx={x} cy={y} r="5" fill="#fff" stroke="#3f9185" strokeWidth="3"/><text x={x} y={Math.max(12, y - 10)} textAnchor="middle" fontSize="10" fill="#315f59">{score}</text><text x={x} y="128" textAnchor="middle" fontSize="9" fill="#7d8987">第 {index + 1} 次</text></g>; })}</svg></div>
  </section>;
}
