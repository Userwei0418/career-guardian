  ...
  return <div className="space-y-7 pb-12">
    ...
    <section className="grid gap-5 xl:grid-cols-2">
      <div className="rounded-3xl border border-[var(--color-border-light)] bg-white p-6"><p className="text-xs font-semibold tracking-[0.18em] text-[var(--color-primary-dark)]">PORTFOLIO</p><h2 className="mt-2 text-2xl font-semibold">作品与成果入口</h2><form onSubmit={createPortfolio} className="mt-5 grid gap-3"><input required maxLength={300} value={portfolio.title} onChange={(event) => setPortfolio({ ...portfolio, title: event.target.value })} placeholder="作品或成果名称" className="rounded-xl border px-3 py-2.5" /><div className="grid gap-3 sm:grid-cols-2"><select aria-label="作品类型" value={portfolio.item_type} onChange={(event) => setPortfolio({ ...portfolio, item_type: event.target.value })} className="rounded-xl border px-3 py-2.5">{portfolioTypes.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select aria-label="关联已确认工作事件" value={portfolio.source_work_event_id} onChange={(event) => setPortfolio({ ...portfolio, source_work_event_id: event.target.value })} className="rounded-xl border px-3 py-2.5"><option value="">不关联工作事件</option>{workspace?.available_work_events.map((item) => <option key={item.id} value={item.id}>{item.occurred_on} · {item.task}</option>)}</select></div><input type="url" value={portfolio.source_url} onChange={(event) => setPortfolio({ ...portfolio, source_url: event.target.value })} placeholder="HTTPS 作品链接（可选）" className="rounded-xl border px-3 py-2.5" /><textarea rows={3} value={portfolio.summary} onChange={(event) => setPortfolio({ ...portfolio, summary: event.target.value })} placeholder="这项成果是什么（可选）" className="rounded-xl border px-3 py-2.5" /><PrivacySelect value={portfolio.privacy_level} onChange={(value) => setPortfolio({ ...portfolio, privacy_level: value })} /><button disabled={Boolean(busy)} className="btn-primary disabled:opacity-50">保存作品草稿</button></form>
        <div className="mt-6 space-y-3">{workspace?.portfolios.length ? workspace.portfolios.map((item) => {
          const analysis = analysisByPortfolio.get(item.id);
          return <article key={item.id} className="rounded-2xl border p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-[var(--color-text-muted)]">{item.item_type} · {privacyLabel[item.privacy_level]} · v{item.version}</p><h3 className="mt-1 font-semibold">{item.title}</h3></div><StatusBadge label={item.status === "active" ? "已确认" : item.status === "draft" ? "草稿" : item.status} confirmed={item.status === "active"} /></div>{item.summary && <p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">{item.summary}</p>}<div className="mt-3 flex flex-wrap gap-2">{item.status === "draft" && <button type="button" disabled={Boolean(busy)} onClick={() => void run(`portfolio-${item.id}`, () => api.patch(`/growth/assets/portfolio/${item.id}`, { expected_version: item.version, status: "active" }), "作品已由你确认。") } className="rounded-lg bg-[var(--color-primary-dark)] px-3 py-2 text-xs text-white">确认作品</button>}<button type="button" disabled={Boolean(busy)} onClick={() => void run(`portfolio-analyze-${item.id}`, () => api.post(`/growth/assets/portfolio/${item.id}/analyze`, { request_id: requestId("portfolio-analysis"), use_ai: true }), "作品分析已保存；能力仍只是候选，需你另行确认。") } className="rounded-lg border border-[var(--color-primary)] px-3 py-2 text-xs text-[var(--color-primary-dark)]">{busy === `portfolio-analyze-${item.id}` ? "分析中…" : analysis ? "重新分析作品" : "分析作品"}</button><button type="button" disabled={Boolean(busy)} onClick={() => { if (window.confirm("删除作品会检查关联证据。若有关联，系统会先阻止删除并说明影响。")) void run(`portfolio-delete-${item.id}`, () => api.delete(`/growth/assets/portfolio/${item.id}`), "作品已删除。"); }} className="rounded-lg border px-3 py-2 text-xs">删除</button><button type="button" disabled={Boolean(busy)} onClick={() => { if (window.confirm("确认解除该作品与证据的关联并删除？失去唯一来源的证据会标记为不可用。")) void run(`portfolio-detach-${item.id}`, () => api.delete(`/growth/assets/portfolio/${item.id}?detach_evidence=true`), "关联已解除，作品已删除；缺少来源的证据已标记不可用。"); }} className="rounded-lg border border-rose-200 px-3 py-2 text-xs text-rose-700">解除关联并删除</button></div>{analysis && <AnalysisCard analysis={analysis} />}</article>;
        }) : <Empty text="还没有作品。可从已确认工作事件或外部链接开始。" />}</div>
      </div>
    ...
    <section className="rounded-3xl border border-[var(--color-border-light)] bg-white p-6 md:p-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold tracking-[0.18em] text-[var(--color-primary-dark)]">CAPABILITY PROFILE</p><h2 className="mt-2 text-2xl font-semibold">能力证据曲线</h2></div><p className="max-w-xl text-sm leading-6 text-[var(--color-text-secondary)]">{workspace?.capability_profile.note}</p></div>{workspace?.capability_profile.axes.length ? <div className="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]"><CapabilityRadar axes={workspace.capability_profile.axes} /><div className="space-y-3">{workspace.capability_profile.axes.map((axis) => <div key={axis.skill_name}><div className="flex justify-between gap-3 text-sm"><span className="font-medium">{axis.skill_name}</span><span className="text-[var(--color-text-muted)]">{axis.evidence_count} 条证据 · {axis.coverage_level}/5</span></div><div className="mt-1.5 h-2 overflow-hidden rounded-full bg-stone-100"><div className="h-full rounded-full bg-[var(--color-primary)]" style={{ width: `${axis.coverage_level * 20}%` }} /></div><p className="mt-1 text-xs text-[var(--color-text-muted)]">{axis.basis}{axis.latest_used_on ? ` · 最近使用 ${axis.latest_used_on}` : ""}</p></div>)}</div></div> : <Empty text="确认能力并关联成长证据后，这里会生成证据覆盖图。" />}{workspace?.capability_profile.timeline.length ? <CapabilityTimeline points={workspace.capability_profile.timeline} /> : null}</section>
...
function CapabilityRadar({ axes }: { axes: CapabilityAxis[] }) {
  ...
  if (shown.length < 3) return <div className="flex min-h-56 items-center justify-center rounded-2xl bg-[var(--color-bg-warm)] p-5 text-center text-sm leading-6 text-[var(--color-text-secondary)]">至少确认 3 项能力后显示雷达形状；当前先展示右侧证据覆盖条。</div>;
