  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <section id="resume-analysis" className="scroll-mt-24 rounded-3xl border border-[var(--color-border-light)] bg-white p-6 md:p-8" aria-labelledby="resume-analysis-title">
        ...
        {guardResult ? <>
          <div className="mt-6 grid gap-5 rounded-2xl bg-[var(--color-bg-warm)] p-5 md:grid-cols-[9rem_1fr] md:items-center"><div><p className="text-xs text-[var(--color-text-muted)]">明示要求匹配度</p><p className="mt-1 text-4xl font-semibold">{guardResult.match_score}%</p></div><div><div className="h-2.5 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[var(--color-primary)]" style={{ width: `${guardResult.match_score}%` }} /></div><p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">{guardResult.summary}</p><p className="mt-1 text-xs text-[var(--color-text-muted)]">{guardResult.analysis_mode === "ai" ? "AI 辅助分析，结论仍需本人确认" : "AI 暂不可用或输出不稳定，当前展示规则核对结果"}{guardResult.reused ? " · 已复用该简历版本的既有分析" : ""}</p></div></div>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><AnalysisList title="已找到的证据" items={[...guardResult.matched_skills, ...guardResult.strengths]} tone="good" /><AnalysisList title="需要补充的证据" items={guardResult.missing_skills} tone="warn" /><AnalysisList title="需要进一步确认" items={guardResult.risks} tone="neutral" /><AnalysisList title="下一步行动" items={guardResult.suggestions} tone="neutral" /></div>
