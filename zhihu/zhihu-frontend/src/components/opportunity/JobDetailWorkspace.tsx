...
export default function JobDetailWorkspace({ jobId, listScore = null }: { jobId: string; listScore?: number | null }) {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <section className="overflow-hidden rounded-3xl border border-[var(--color-border-light)] bg-white shadow-sm">
        <div className="grid lg:grid-cols-[1fr_22rem]">
          ...
          <aside className="border-t border-[var(--color-border-light)] bg-[var(--color-bg-warm)] p-6 lg:border-l lg:border-t-0 lg:p-7">
            ...
            {listScore != null && <div className="mt-4 rounded-xl bg-white px-3 py-2 text-xs text-[var(--color-text-secondary)]"><span className="font-semibold text-[var(--color-primary-dark)]">列表初筛相关度 {listScore}%</span><span className="ml-1">详情分析会在相同权重下继续核对硬门槛。</span></div>}
      ...
      <section id="resume-analysis" className="scroll-mt-24 rounded-3xl border border-[var(--color-border-light)] bg-white p-6 md:p-8" aria-labelledby="resume-analysis-title">
        ...
        {guardResult ? <>
          <div className="mt-6 grid gap-5 rounded-2xl bg-[var(--color-bg-warm)] p-5 md:grid-cols-[10rem_1fr] md:items-center"><div><p className="text-xs text-[var(--color-text-muted)]">综合证据匹配度</p><p className="mt-1 text-4xl font-semibold">{guardResult.match_score}%</p><p className="mt-1 text-[11px] text-[var(--color-text-muted)]">不是录用概率</p></div><div><div className="h-2.5 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[var(--color-primary)]" style={{ width: `${guardResult.match_score}%` }} /></div><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white px-2.5 py-1">方向 {guardResult.score_breakdown.direction ?? 0}/35</span><span className="rounded-full bg-white px-2.5 py-1">背景门槛 {guardResult.score_breakdown.background ?? 0}/30</span><span className="rounded-full bg-white px-2.5 py-1">技能证据 {guardResult.score_breakdown.skills ?? 0}/35</span>{(guardResult.score_breakdown.unmet_gates?.length ?? 0) > 0 && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-amber-800">待确认：{guardResult.score_breakdown.unmet_gates?.join("、")}</span>}</div><p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">{guardResult.summary}</p><p className="mt-1 text-xs text-[var(--color-text-muted)]">统一评分由系统按固定权重计算，AI 只解释证据和建议，不能把分数改成 100% 或 0%。</p></div></div>
