...
| 导航 | 作用 |
| --- | --- |
| 今天 | 当前建议、正在进行的任务、重要提醒，以及下半部的六阶段长纵向时间线 |
| 一起看看 | 发起 Offer、合同、薪资、工资条等任务 |
| 我的档案 | 管理个人情况、材料、报告及隐私设置 |

"职位市场"和"知识学堂"不建议首期作为一级导航。市场数据应嵌入具体任务，知识内容应在用户遇到相应问题时出现。原 `/journey` 用户页面和导航入口已移除；六阶段内容并入 `/today` 下半部分，守护记录与职业事件承担连续任务和完成状态的串联。
...
欢迎和轻量身份引导；陪伴式首页；首页下半部六阶段长纵向时间线；Offer 上传、粘贴及手动录入；Offer 信息识别和用户确认；单份 Offer 分析；两份 Offer 对比；税后收入和城市生活结余计算；招聘市场薪资水平对比；HR 待确认问题及沟通话术；劳动合同上传、解析和风险说明；Offer 与合同一致性检查；签约前行动清单；守护记录与职业事件连续链路；用户数据删除及演示模式。
...
_下一步行动_——按钮：生成 HR 提问清单、和另一份 Offer 比较、纳入守护记录、准备检查合同。
...
### **9.17 今天页下半部：六阶段职业时间线**

**页面目标**：在 `/today` 下半部分提供从在校、求职、签约、入职、理财到跳槽/成长的长期内容入口，不再维护独立 `/journey` 用户页面。

**页面布局**：采用长纵向时间线，依次展开六个阶段。每个阶段完整展示当前真实已发布的知识文章与现有工具入口；内容随发布状态动态变化，不写死文章数、工具数，也不把条目数量表达为用户进度。

**交互**：文章打开真实文章内容，工具进入已有业务路由。与个人相关的连续状态不由文章浏览量推断，而由守护记录和职业事件中的事实、结论、待办行动与结果承接。系统仍根据当前守护状态只推荐一个主要行动。

**兼容边界**：后端 `/api/journey`、`JourneyNode` 和历史迁移暂时保留为兼容层；本期只移除用户页面与导航，不声称兼容接口或历史数据结构已经退役。
...
├── profiles/
├── events/guardian/
├── journeys/（兼容 API 与历史 JourneyNode）
├── cases/
│   ├── offers/
│   ├── contracts/
│   └── payslips/
...
其中：`profiles` 管理用户阶段、偏好和目标；`events/guardian` 以守护记录和职业事件管理连续事实、结论、行动与结果；`journeys` 仅保留 `/api/journey`、`JourneyNode` 和历史迁移兼容，不再对应独立用户页面；`offers` 负责 Offer 录入、提取和比较；`contracts` 负责合同解释和一致性检查；`payslips` 负责工资条识别和差异分析；`documents` 负责上传、OCR 和文件删除；`calculators` 负责薪资、税务和生活成本；`rules` 负责确定性条款规则；`market` 调用职涯通数据；`reports` 生成统一职护报告；`assistant` 负责 AI 追问、解释和话术生成。
...
**第一阶段：产品原型与设计系统**——完成用户旅程、信息架构、视觉规范、首页原型、首页下半部六阶段长纵向时间线、Offer 分步流程、Offer 报告页、合同阅读页、演示用户故事。这一阶段使用模拟数据，不急于连接全部后端。
...
**第四阶段：长期陪伴和比赛优化**——完善守护记录与职业事件连续链路、入职提醒、工资条核对、隐私设置、演示模式、降级数据、项目说明书、演示视频和答辩材料。

---

## **十九、项目验收标准**

首个可参赛版本至少应满足：用户可从首页进入 Offer 分析；首页下半部以六阶段长纵向时间线完整展示当前真实已发布的知识文章和现有工具入口；可通过上传、粘贴或手动方式添加 Offer；系统提取内容后允许用户确认和修改；可计算预估到手收入和生活结余；可展示招聘市场薪资位置；可比较两份 Offer；可生成有依据的 HR 问题；可上传并解释劳动合同；可发现 Offer 与合同的主要差异；可生成签约前清单；所有重要结论可查看来源；用户可删除上传材料；提供稳定的比赛演示模式；整体页面保持陪伴式、分步骤、非后台化风格。
...
| 9.17 今天页六阶段职业时间线 | 已发布知识文章＋现有工具入口；连续状态由守护记录和职业事件承接 | KnowledgeArticle / CareerEvent | P0 |
