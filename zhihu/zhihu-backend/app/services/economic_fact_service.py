...
def merge_fact_evidence_locked(
    db: Session,
    *,
    user_id: int,
    ...
    The caller owns the transaction boundary and must hold the per-user ledger
    lock. Keeping the invariant checks here lets ordinary ledger review and
    import confirmation use exactly the same merge rules.
    """
    if primary_transaction.id == evidence_transaction.id:
        raise HTTPException(status_code=400, detail="不能把同一条记录重复合并")
    if (
        primary_transaction.user_id != user_id
        or evidence_transaction.user_id != user_id
    ):
    ...
    if allocated_amount > min(Decimal(primary_fact.amount), available_amount):
    ...
    if target_allocation is None:
        ...
        target_allocation.confirmed_by_user_id = user_id
