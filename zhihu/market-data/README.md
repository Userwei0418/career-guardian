...
正常采集默认使用后台无头 Chromium，因此本机不会弹出可见浏览器窗口。分页不是某家公司的临时脚本：公司渠道统一声明 `pagination.mode`，采集引擎支持 `infinite_scroll`（下滚加载）、`load_more`（点击继续加载）、`next_button`（点击下一页）、`auto`（按页面能力自动判断）和 API 请求分页。每个渠道仍可配置按钮选择器、加载等待、最大批次和最大记录数，但加载循环、去重、停止原因和审计日志由平台统一实现，避免把“首批 20 条”误当作完整采集。

周期性采集默认采用数据级增量边界，而不是记录容易漂移的页码。首次成功执行会完整扫描并保存近期稳定岗位标识；后续从最新一批开始，当整批命中已知标识时停止。只有采集、字段映射和质量门成功完成后才推进 `source_collection_checkpoints`，失败或隔离不会错误越过边界。默认每连续 10 次增量执行一次全量回扫，用于发现站点乱序、旧岗位更新或入口变更。管理页会展示本次是全量还是增量、边界版本、最近标识数量、加载批次和停止原因。
