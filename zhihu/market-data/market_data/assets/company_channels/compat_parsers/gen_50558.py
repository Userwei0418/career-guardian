...
def extract_table_from_html(htmlcontext, tempfile):
    ...
    for card in soup.find_all('div', class_='card-item-wrap'):
        ...
        hd_dept = card.find('span', class_='pos-summary').get_text(strip=True) if card.find('span', class_='company-name') else ""
        ...
        hd_job_category =  card.find('span', class_='pos-cate').get_text(strip=True) if card.find('span', class_='pos-cate') else "" # Assuming no job category is provided in the HTML
