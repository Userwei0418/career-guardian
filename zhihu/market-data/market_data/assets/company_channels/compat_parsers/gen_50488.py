...
def extract_table_from_html(htmlcontext, tempfile):
    ...
    for card in soup.find_all('div', class_='container-aOp138AX_X'):
        ...
        hd_dept = card.find('div', class_='no-adaptive-tooltip').get_text(strip=True) if card.find('div', class_='sd-foundation-body-secondary-1Z7H-') else ""
        hd_loc = card.find_all('div', class_='no-adaptive-tooltip')[3].get_text(strip=True) if len(card.find_all('div', class_='sd-foundation-body-secondary-1Z7H-')) > 3 else ""
        hd_job_num = ""  # Assuming there's no job number in the provided HTML
        hd_job_category = ""  # Assuming there's no job category in the provided HTML
        if hd_loc == "":
            hd_loc = "其他"
