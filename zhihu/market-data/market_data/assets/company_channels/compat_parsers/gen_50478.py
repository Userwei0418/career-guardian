...
def extract_table_from_html(htmlcontext, tempfile):
    ...
    for item in job_items:
        ...
        hd_dept = item.find("div", class_="style__STLabelText-editor__sc-10r1nhd-13").get_text(strip=True) if item.find("div", class_="style__STLabelText-editor__sc-10r1nhd-13") else ""
        hd_loc = item.find_all("div", class_="style__STLabelText-editor__sc-10r1nhd-13")[1].get_text(strip=True) if len(item.find_all("div", class_="style__STLabelText-editor__sc-10r1nhd-13")) > 1 else ""
        hd_job_num = ""  # Assuming no job number is provided in the HTML
        hd_job_category = item.find("div", class_="style__STLabelText-editor__sc-10r1nhd-13").get_text(strip=True) if item.find("div", class_="style__STLabelText-editor__sc-10r1nhd-13") else ""
